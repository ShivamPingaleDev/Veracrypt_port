...
The SwiftUI app uses the same `vc_mobile` C API. Fingerprint / Face ID unlock is off unless `VCPortEnableBiometrics` is true (this branch). `experimental-biometrics` is **stale**. A personal C CLI that links the same `libvc_mobile` is `experimental-pure-c` (`ports/pure-c/`).
