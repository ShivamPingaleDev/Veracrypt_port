...
The SwiftUI app uses the same `vc_mobile` C API. Fingerprint / Face ID unlock is off unless `VCPortEnableBiometrics` is true (`experimental-otg-master` and `experimental-biometrics`).
