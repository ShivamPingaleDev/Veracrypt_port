...
- “VC Port 0.3.11 is a stable alpha. GitHub APKs are debug-signed previews; you sign production.”
...
| Android (stable alpha) | GitHub Release `v0.3.11` — one debug-signed FOSS APK (`VCPort-0.3.11.apk`) |
| iPhone (stable alpha) | GitHub Release `v0.3.11` unsigned IPA (`VCPort-0.3.11-unsigned-preview.ipa`). Sign it yourself (see [FOSS.md](FOSS.md) / [ios/README.md](ios/README.md)). |
...
2. Download the unsigned IPA from the GitHub Release (`VCPort-0.3.11-unsigned-preview.ipa`).
...
- Keep releases tagged (`v0.3.11`) so “VC Port 0.3.11” is a stable URL.
