#!/bin/sh
# Boot an iPad Simulator and install Debug VC Port (no Apple ID).
# Device sideload (your name on the cert): ports/ios/sideload-sign.sh
# Skips when Xcode Simulator runtimes are missing (CI, sandbox).
set -eu
IOS="$(CDPATH= cd -- "$(dirname "$0")" && pwd)"
DERIVED="${IOS}/build/DerivedData"
SHOTS="$(CDPATH= cd -- "$IOS/../docs/screenshots" && pwd)"
...
APP=$(find "$DERIVED" -name 'VCPort.app' -path '*/Debug-iphonesimulator/*' | head -n 1)
if [ -z "$APP" ] || [ ! -d "$APP" ]; then
	echo "FAIL  VCPort.app not in $DERIVED"
	exit 1
fi
