...
These PNGs are **real** production debug UI from emulator AVD `vcport-api35`, captured with Compose `captureToImage` while **FLAG_SECURE stays on**. `adb screencap` is black on purpose (seizure / recents).
