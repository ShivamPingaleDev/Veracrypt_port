...
class SlowHumanSessionTest {
    ...
    fun generateCopyMinimizePasteContinueAndBasketSize() {
        ...
        rule.onNodeWithTag("copy_once").performScrollTo().performClick()
        rule.waitForIdle()
        assertEquals("Create password must survive Home so the wizard can continue", secret, rule.activity.testingCreatePassword())
    ...
    fun nestedCreateMinimizeKeepsWizard() {
        ...
        rule.onNodeWithTag("copy_once").performScrollTo().performClick()
        rule.waitForIdle()
        val outer = rule.activity.testingCreatePassword()
        assertEquals("outer Copy once", 64, outer.length)
        ...
        rule.waitForIdle()
        val nested = rule.activity.testingCreateHiddenPassword()
        assertEquals("nested Copy once", 64, nested.length)
        ...
        rule.onNodeWithTag("copy_once").performScrollTo().performClick()
        rule.waitForIdle()
        assertEquals("outer password must survive Home", outer, rule.activity.testingCreatePassword())
        slow()
        rule.onNodeWithTag("copy_nested_once").performScrollTo().performClick()
        rule.waitForIdle()
        assertEquals("nested password must survive Home", nested, rule.activity.testingCreateHiddenPassword())
        ...
        rule.onNodeWithTag("copy_once").performScrollTo().performClick()
        rule.waitForIdle()
        assertEquals(outer, rule.activity.testingCreatePassword())
        rule.onNodeWithTag("copy_nested_once").performScrollTo().performClick()
        rule.waitForIdle()
        assertEquals(nested, rule.activity.testingCreateHiddenPassword())
        rule.onNodeWithText("Create volume").performScrollTo().assertIsEnabled()
        assertEquals(outer + "\n" + nested, notes.readText())
        Hardening.wipeDir(dir)
    }
